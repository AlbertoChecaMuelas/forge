...
#      closure PR opened with auto-merge enabled, or skipped silently)
#   2  FORGE_VERSION could not be parsed from install.sh
#   4  git push of the tag failed
#   5  git push of the CHANGELOG closure branch failed
#   7  gh pr merge --auto failed
#
set -euo pipefail
...
BRANCH_NAME="release/${TAG}-changelog"
echo "auto-tag: creating branch ${BRANCH_NAME}"
if ! git checkout -b "${BRANCH_NAME}"; then
  echo "auto-tag: failed to create branch ${BRANCH_NAME}" >&2
  exit 5
fi

if ! git push origin "${BRANCH_NAME}"; then
  echo "auto-tag: git push of branch ${BRANCH_NAME} failed" >&2
  exit 5
fi

PR_TITLE="docs(changelog): close [Unreleased] as ${TAG}"
PR_BODY="Auto-generated by release job. Closes [Unreleased] in CHANGELOG.md as ${TAG}.

This PR will auto-merge once the required \`test\` status check passes."

echo "auto-tag: opening PR ${BRANCH_NAME} -> ${DEFAULT_BRANCH}"
if ! gh pr create \
    --base "${DEFAULT_BRANCH}" \
    --head "${BRANCH_NAME}" \
    --title "${PR_TITLE}" \
    --body "${PR_BODY}" 2>&1; then
  echo "auto-tag: gh pr create returned non-zero (PR may already exist); continuing"
fi

echo "auto-tag: enabling auto-merge (squash) on PR for ${BRANCH_NAME}"
if ! gh pr merge --auto --squash "${BRANCH_NAME}" 2>&1; then
  echo "auto-tag: gh pr merge --auto failed" >&2
  exit 7
fi

echo "auto-tag: PR ${BRANCH_NAME} will auto-merge once 'test' check passes"
