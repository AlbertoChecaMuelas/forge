...
allowed-tools: Bash(git log *) Bash(git diff *) Bash(git rev-parse *) Bash(ROOT="$(git rev-parse --show-toplevel)"; if [ -f "$ROOT/tools/release/bump-version.sh" ] && grep -q '^FORGE_VERSION=' "$ROOT/install.sh" 2>/dev/null; then bash "$ROOT/tools/release/mr-stamp.sh" *) Write
context: fork
---
...
!`ROOT="$(git rev-parse --show-toplevel)"; if [ -f "$ROOT/tools/release/bump-version.sh" ] && grep -q '^FORGE_VERSION=' "$ROOT/install.sh" 2>/dev/null; then bash "$ROOT/tools/release/mr-stamp.sh" --base ${1:-master}; fi`
