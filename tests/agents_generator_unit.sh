...
ARSENAL_ROOT="$(pwd)"
GENERATOR="$ARSENAL_ROOT/shared/scripts/generate-agents.sh"
...
test_claude_check_detects_mutation() {
  local frontmatter_dir="$ARSENAL_ROOT/shared/scripts/claude-frontmatter"
  local role="applier"
...
test_claude_frontmatter_well_formed() {
  local agents_dir="$ARSENAL_ROOT/agents"
...
test_plan_format_skill_frontmatter() {
  local skill_file="$ARSENAL_ROOT/skills/plan-format/SKILL.md"
...
test_tester_skills_three_way_invariant() {
  local tester_body="$ARSENAL_ROOT/shared/agents/tester.body.md"
  local commands_json="$ARSENAL_ROOT/shared/components/commands.json"
  ...
  for skill in $skill_names; do
    ...
    local full_path="$ARSENAL_ROOT/$skill_path"
