...
test_token_requires_plan_senior() {
  if grep -qF 'REQUIRES_PLAN:' "$FORGE_ROOT/agents/senior.md"; then
...
test_token_blocked_senior_senior() {
  if grep -qF 'BLOCKED_SENIOR:' "$FORGE_ROOT/agents/senior.md"; then
...
test_token_blocked_applier() {
  if grep -qF 'BLOCKED:' "$FORGE_ROOT/agents/applier.md"; then
...
test_token_verifier_failed_applier() {
  if grep -qF 'VERIFIER_FAILED:' "$FORGE_ROOT/agents/applier.md"; then
...
test_token_escalate_senior_tech() {
  if grep -qF 'ESCALATE_SENIOR:' "$FORGE_ROOT/agents/tech.md"; then
...
test_token_findings_phase_reviewer() {
  if grep -qF 'FINDINGS_PHASE:' "$FORGE_ROOT/skills/execute-plan/reference/review-template.md"; then
...
test_token_ok_phase_reviewer() {
  if grep -qF 'OK_PHASE:' "$FORGE_ROOT/skills/execute-plan/reference/review-template.md"; then
...
test_token_verified_reviewer() {
  if grep -qF 'VERIFIED:' "$FORGE_ROOT/skills/execute-plan/reference/review-template.md"; then
...
test_token_staged_senior() {
  local file1="$FORGE_ROOT/agents/senior.md"
...
test_disallowed_tools_senior() {
  local f1="$FORGE_ROOT/agents/senior.md"
...
test_label_phase_create_plan() {
  if grep -qF '# PHASE' "$FORGE_ROOT/skills/create-plan/reference/plan-format.md"; then
...
test_label_step_create_plan() {
  if grep -qF '## Step' "$FORGE_ROOT/skills/create-plan/reference/plan-format.md"; then
...
test_label_checkpoint_phase_create_plan() {
  if grep -qF '### CHECKPOINT PHASE' "$FORGE_ROOT/skills/create-plan/reference/plan-format.md"; then
...
test_label_global_verifier_create_plan() {
  if grep -qF '# GLOBAL VERIFIER' "$FORGE_ROOT/skills/create-plan/reference/plan-format.md"; then
...
test_label_rollback_create_plan() {
  if grep -qF '# ROLLBACK' "$FORGE_ROOT/skills/create-plan/reference/plan-format.md"; then
...
test_label_risks_verified_execute_plan() {
  if grep -qF '## Risks verified by reviewer' "$FORGE_ROOT/skills/execute-plan/SKILL.md"; then
...
test_blocked_staging_tokens_in_skill() {
  local skill="$FORGE_ROOT/skills/create-plan/SKILL.md"
...
test_mkdir_plans_before_step1_in_skill() {
  if awk '/mkdir -p \.plans\//{mk=NR} /^### Step 1 /{s1=NR; exit} END{exit !(mk && s1 && mk < s1)}' \
      "$FORGE_ROOT/skills/create-plan/SKILL.md"; then
...
test_role_boundary_agents() {
  ...
  for agent in $AGENTS_ROLE_BOUNDARY; do
    local file="$FORGE_ROOT/agents/${agent}.md"
...
REVIEWER_REF="$FORGE_ROOT/skills/execute-plan/reference/reviewer-and-close.md"
EXECUTE_SKILL="$FORGE_ROOT/skills/execute-plan/SKILL.md"
ESCALATION_CODES="$FORGE_ROOT/shared/reference/escalation-codes.md"
...
test_escalation_codes_table_parseable() {
  local rows
  rows="$(awk -F'|' 'NF>2 && $2 ~ /`[A-Z_]+:/' "$FORGE_ROOT/shared/reference/escalation-codes.md" | wc -l | tr -d ' ')"
  if [ "$rows" -ge 18 ]; then
...
test_escalation_codes_emitter_crosscheck() {
  local codes_file="$FORGE_ROOT/shared/reference/escalation-codes.md"
  ...
  while IFS="	" read -r token emitter_csv; do
    ...
    for emitter in "${emitters_arr[@]}"; do
      ...
      case "$emitter" in
        applier)
          files=("$FORGE_ROOT/agents/applier.md") ;;
        tech)
          files=("$FORGE_ROOT/agents/tech.md") ;;
        tester)
          files=("$FORGE_ROOT/agents/tester.md") ;;
        senior)
          files=("$FORGE_ROOT/agents/senior.md") ;;
        reviewer)
          files=("$FORGE_ROOT/skills/execute-plan/reference/review-template.md") ;;
        *)
      ...
      for f in "${files[@]}"; do
        if grep -qF -- "$token" "$f"; then
          pass "escalation_codes_emitter_crosscheck: '$token' found in ${f#$FORGE_ROOT/}"
        else
          fail "escalation_codes_emitter_crosscheck: '$token' NOT found in ${f#$FORGE_ROOT/}"
        fi
      done
    done
  done <<< "$pairs"
...
test_escalation_codes_orchestrator_routing_coverage() {
  local codes_file="$FORGE_ROOT/shared/reference/escalation-codes.md"
  local orch_file="$FORGE_ROOT/shared/CLAUDE-orchestrator.md"
