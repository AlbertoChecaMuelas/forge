...
# tests/settings_integration.sh — Integration tests for arsenal_install_settings
...
SHARED_SETTINGS="$ARSENAL_ROOT/shared/settings.shared.json"
...
# =============================================================================
test_install_creates_pre_arsenal_backup() {
  local h
  h="$(make_home)"
  ...
  if [ ! -f "$pre" ]; then
    fail "creates_pre_arsenal_backup: .pre-forge not created"
    return
  fi
  pass "creates_pre_arsenal_backup: .pre-forge exists"
  ...
  if diff -q "$orig_tmp" "$pre" >/dev/null 2>&1; then
    pass "creates_pre_arsenal_backup: .pre-forge content matches original"
  else
    fail "creates_pre_arsenal_backup: .pre-forge content does NOT match original"
  fi
  rm -f "$orig_tmp"
}
...
# =============================================================================
test_install_aborts_on_corrupt_existing_settings_no_pre_arsenal() {
  local h
  h="$(make_home)"
  ...
  if [ "$exit_code" -ne 0 ]; then
    pass "corrupt_no_pre_arsenal: exit != 0 as expected"
  else
    fail "corrupt_no_pre_arsenal: expected exit != 0 but got 0"
  fi

  # State file must NOT have been created (install was aborted)
  if [ ! -f "$h/.forge-state.json" ]; then
    pass "corrupt_no_pre_arsenal: state file not created"
  else
    fail "corrupt_no_pre_arsenal: state file was created despite abort"
  fi
}
...
# =============================================================================
test_install_restores_from_pre_arsenal_when_target_corrupt() {
  local h
  h="$(make_home)"
  ...
  local pre="${sf}.pre-forge"
  if [ ! -f "$pre" ]; then
    fail "restore_from_pre_arsenal: .pre-forge not created by first install (precondition)"
    return
  fi
  ...
  if [ "$exit_code" -eq 0 ]; then
    pass "restore_from_pre_arsenal: exit 0 after restoring from .pre-forge"
  else
    fail "restore_from_pre_arsenal: exit $exit_code, expected 0"
    return
  fi

  # Result must be valid JSON
  if jq empty "$sf" 2>/dev/null; then
    pass "restore_from_pre_arsenal: settings.json is valid JSON after restore+merge"
  else
    fail "restore_from_pre_arsenal: settings.json is invalid JSON after restore+merge"
  fi
}
...
run_test test_install_creates_pre_arsenal_backup
run_test test_install_idempotent_no_double_backup
run_test test_state_file_has_overlay_backup
run_test test_install_aborts_on_corrupt_existing_settings_no_pre_arsenal
run_test test_install_restores_from_pre_arsenal_when_target_corrupt
