...
test_format_two_tab_fields() {
  local output
  output=$(FORGE_ROOT="$FORGE_ROOT" forge_symlink_catalog)
  local bad
...
test_sources_exist() {
  ...
  while IFS=$'\t' read -r src _; do
    if [ ! -f "$FORGE_ROOT/$src" ]; then
      fail "Source not found: $FORGE_ROOT/$src"
      all_ok=0
    fi
  done <<< "$output"
  if [ "$all_ok" -eq 1 ]; then
    pass "sources_exist: all source files exist relative to FORGE_ROOT"
  fi
}
...
test_no_duplicate_destinations() {
  local output
  output=$(FORGE_ROOT="$FORGE_ROOT" forge_symlink_catalog | sort -u)
  local dups
...
test_components_list_nine_known() {
  local output
  output=$(FORGE_ROOT="$FORGE_ROOT" forge_components_list | sort)
  local count
  count=$(echo "$output" | wc -l | tr -d ' ')
  ...
  while IFS= read -r name; do
    if [ ! -f "$FORGE_ROOT/shared/components/${name}.json" ]; then
...
test_components_default_list_excludes_core() {
  local output
  output=$(FORGE_ROOT="$FORGE_ROOT" forge_components_default_list | sort)
  local expected
...
test_component_symlinks_known_format() {
  local output
  output=$(FORGE_ROOT="$FORGE_ROOT" forge_component_symlinks agents)
  ...
  # Every src path must exist under FORGE_ROOT
  local all_ok=1
  while IFS=$'\t' read -r src _; do
    if [ ! -f "$FORGE_ROOT/$src" ]; then
      fail "component_symlinks: src not found: $FORGE_ROOT/$src"
      all_ok=0
    fi
  done <<< "$output"
  if [ "$all_ok" -eq 1 ]; then
    pass "component_symlinks: all src paths exist under FORGE_ROOT"
  fi
}
...
test_component_symlinks_unknown_exits_nonzero() {
  local result=0
  FORGE_ROOT="$FORGE_ROOT" forge_component_symlinks no-such-component >/dev/null 2>&1 || result=$?
  if [ "$result" -ne 0 ]; then
...
test_component_symlinks_commands_has_reference_entries() {
  local output
  output=$(FORGE_ROOT="$FORGE_ROOT" forge_component_symlinks commands)
