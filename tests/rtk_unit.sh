...
# We source rtk.sh in a script fragment that also sets ARSENAL_ROOT.
...
export ARSENAL_ROOT="$ARSENAL_ROOT"
# Load rtk.sh
...
test_decide_absent_invokes_install() {
  ...
  # Override HOME so that forge_rtk_detect's on-disk fallback (~/.arsenal/bin/rtk)
...
SCRIPTEOF
  ...
  local pinned
  pinned="$(cat "$ARSENAL_ROOT/rtk/VERSION")"
...
# ---------------------------------------------------------------------------
# Test 11: decide gt, ARSENAL_RTK_ADJUST=yes → downgrade called, mismatch empty
# ---------------------------------------------------------------------------
...
forge_rtk_adjust_via_tarball() {
  local p
  p="$(cat "$ARSENAL_ROOT/rtk/VERSION")"
...
'
  ...
  local output
  output="$(_run_decide_script "$bin_dir" "$extra_code" "ARSENAL_RTK_ADJUST=yes")"
...
# ---------------------------------------------------------------------------
# Test 12: decide gt, ARSENAL_RTK_ADJUST=no → mismatch=1, downgrade NOT called
# ---------------------------------------------------------------------------
...
"
  ...
  local output
  output="$(_run_decide_script "$bin_dir" "$extra_code" "ARSENAL_RTK_ADJUST=no")"
...
# ---------------------------------------------------------------------------
# Test 13: decide gt, ARSENAL_RTK_ADJUST=maybe → falls back to no-tty → mismatch=1
# ---------------------------------------------------------------------------
...
export PATH="$bin_dir:/usr/bin:/bin"
export ARSENAL_ROOT="$ARSENAL_ROOT"
export ARSENAL_RTK_ADJUST=maybe
unset RTK_FORCE_TTY 2>/dev/null || true
source "$ARSENAL_ROOT/lib/rtk.sh"
forge_rtk_decide 2>/dev/null || true
echo "_RTK_VERSION_MISMATCH=\${_RTK_VERSION_MISMATCH:-}"
...
# ---------------------------------------------------------------------------
# Test 13b: decide gt, ARSENAL_RTK_DOWNGRADE=yes (deprecated alias)
...
'
  ...
  local output
  output="$(_run_decide_script "$bin_dir" "$extra_code" "ARSENAL_RTK_DOWNGRADE=yes")"
...
# Both ARSENAL_RTK_ADJUST and ARSENAL_RTK_DOWNGRADE are unset.
# ---------------------------------------------------------------------------
...
SHIM
  ...
  # Run with stdin from /dev/null (no tty) and no ARSENAL_RTK_ADJUST / ARSENAL_RTK_DOWNGRADE env var
  local script
  script="$(mktemp /tmp/rtk_test_XXXX.sh)"
  cat > "$script" <<SCRIPTEOF
#!/usr/bin/env bash
...
export ARSENAL_ROOT="$ARSENAL_ROOT"
unset RTK_FORCE_TTY 2>/dev/null || true
unset ARSENAL_RTK_ADJUST 2>/dev/null || true
unset ARSENAL_RTK_DOWNGRADE 2>/dev/null || true
source "$ARSENAL_ROOT/lib/rtk.sh"
forge_rtk_decide 2>/dev/null || true
echo "_RTK_INSTALLED_BY_US=\${_RTK_INSTALLED_BY_US:-}"
echo "_RTK_INSTALL_FAILED=\${_RTK_INSTALL_FAILED:-}"
...
# No rtk shim (absent from PATH), no ARSENAL_RTK_ADJUST, stdin from /dev/null (no-tty).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Test B2: decide absent, ARSENAL_RTK_ADJUST=yes → calls adjust stub,
