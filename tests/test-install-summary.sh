...
echo "WARN_COUNT=$_FORGE_WARN_COUNT"

# Call forge_err once
forge_err "err msg" 2>/dev/null
echo "ERR_COUNT=$_FORGE_ERR_COUNT"

# Reset and verify
_forge_reset_summary
echo "WARN_AFTER_RESET=$_FORGE_WARN_COUNT"
echo "ERR_AFTER_RESET=$_FORGE_ERR_COUNT"
'
  local output
  ...
  assert_eq "1" "$warn_count" "Case A: _FORGE_WARN_COUNT should be 1 after forge_warn" || return 1
  assert_eq "1" "$err_count" "Case A: _FORGE_ERR_COUNT should be 1 after forge_err" || return 1
  assert_eq "0" "$warn_after" "Case A: _FORGE_WARN_COUNT should be 0 after reset" || return 1
  assert_eq "0" "$err_after" "Case A: _FORGE_ERR_COUNT should be 0 after reset" || return 1
}
