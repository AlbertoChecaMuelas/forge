...
# ARSENAL_SOURCED_FOR_TESTS so install.sh main body is skipped.
...
_source_clean_target() {
  ...
  # We also need ARSENAL_ROOT set to the fake value inside each test, so we
  # define a thin wrapper that sets ARSENAL_ROOT before calling the real fn.
  local fn_body
  fn_body="$(awk '/^_forge_clean_target\(\)/{found=1} found{print} found && /^\}$/{exit}' \
    "$ARSENAL_ROOT/install.sh")"
  eval "$fn_body"
}
...
# 1. Symlink pointing into ${ARSENAL_ROOT}-other/ must NOT be removed (prefix collision)
test_prefix_collision_not_removed() {
  local tmp
  tmp="$(make_tmp)"

  # fake ARSENAL_ROOT and sibling directory
  local fake_root="$tmp/arsenal"
  local fake_sibling="$tmp/arsenal-other"
  mkdir -p "$fake_root"
  mkdir -p "$fake_sibling"

  # File inside the sibling (not owned by arsenal)
  ...
  # Run with ARSENAL_ROOT overridden to the fake path
  ARSENAL_ROOT="$fake_root" _forge_clean_target "$fake_home" >/dev/null 2>&1
...
# 2. Symlink pointing into ${ARSENAL_ROOT}/... MUST be removed (owned by arsenal)
test_owned_symlink_removed() {
  ...
  if [ ! -L "$fake_home/agents/agent.md" ]; then
    pass "owned_symlink_removed: symlink to arsenal root was removed"
  else
    fail "owned_symlink_removed: symlink to arsenal root was NOT removed"
  fi
}
...
# 5. skills/<name>/SKILL.md symlink pointing into ARSENAL_ROOT MUST be removed
