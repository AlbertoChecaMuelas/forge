...
test_uc1_classification() {
  (
    TMPDIR_UC="$ARSENAL_ROOT/tests/.tmp/uc1_$$"
    mkdir -p "$TMPDIR_UC"
...
test_uc2_scope_stripping() {
  (
    TMPDIR_UC="$ARSENAL_ROOT/tests/.tmp/uc2_$$"
    mkdir -p "$TMPDIR_UC"
    trap 'rm -rf "$TMPDIR_UC"' EXIT
    cd "$TMPDIR_UC"
...
test_uc3_additive() {
  (
    TMPDIR_UC="$ARSENAL_ROOT/tests/.tmp/uc3_$$"
    mkdir -p "$TMPDIR_UC"
    trap 'rm -rf "$TMPDIR_UC"' EXIT
    cd "$TMPDIR_UC"
...
test_uc4_noop() {
  (
    TMPDIR_UC="$ARSENAL_ROOT/tests/.tmp/uc4_$$"
    mkdir -p "$TMPDIR_UC"
    trap 'rm -rf "$TMPDIR_UC"' EXIT
    cd "$TMPDIR_UC"
...
test_uc5_patch_staging() {
  (
    TMPDIR_UC="$ARSENAL_ROOT/tests/.tmp/uc5_$$"
    mkdir -p "$TMPDIR_UC"
    trap 'rm -rf "$TMPDIR_UC"' EXIT
    cd "$TMPDIR_UC"
...
test_uc6_none_staging() {
  (
    TMPDIR_UC="$ARSENAL_ROOT/tests/.tmp/uc6_$$"
    mkdir -p "$TMPDIR_UC"
    trap 'rm -rf "$TMPDIR_UC"' EXIT
    cd "$TMPDIR_UC"
...
test_mr_description_gitignored() {
  if grep -qxF 'PR-DESCRIPTION.md' "$ARSENAL_ROOT/.gitignore"; then
...
test_mr_description_not_tracked() {
  if git -C "$ARSENAL_ROOT" ls-files --error-unmatch PR-DESCRIPTION.md >/dev/null 2>&1; then
