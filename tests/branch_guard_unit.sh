...
BRANCH_GUARD="$ARSENAL_ROOT/shared/branch-guard.sh"
...
cleanup() {
  ...
  rm -rf "${TMPDIR:-/tmp}/atenea-branch-guard/" 2>/dev/null || true
}
trap cleanup EXIT
...
test_merged_branch() {
  ...
  rm -f "${TMPDIR:-/tmp}/atenea-branch-guard/${repo_hash}.shown"
...
# (g) ATENEA_BRANCH_GUARD_DISABLE=1 → exit 0, sin output
test_disable_env_var() {
  ...
  run_guard "$local_repo" "ATENEA_BRANCH_GUARD_DISABLE=1"
