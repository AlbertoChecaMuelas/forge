...
AGENTS_JSON="$ARSENAL_ROOT/shared/components/agents.json"
STATUSLINE_JSON="$ARSENAL_ROOT/shared/components/statusline.json"
CORE_JSON="$ARSENAL_ROOT/shared/components/core.json"
COST_REPORT_SKILL_JSON="$ARSENAL_ROOT/shared/components/cost-report-skill.json"
COST_REPORT_JSON="$ARSENAL_ROOT/shared/components/cost-report.json"
...
test_cost_report_skill_symlink_target() {
  ...
  # The source file must actually exist under ARSENAL_ROOT
  if [ -f "$ARSENAL_ROOT/$src" ]; then
    pass "cost-report-skill.json: symlink src file exists at $ARSENAL_ROOT/$src"
  else
    fail "cost-report-skill.json: symlink src file missing at $ARSENAL_ROOT/$src"
  fi
}
