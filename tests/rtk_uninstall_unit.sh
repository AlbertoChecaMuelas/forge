...
# TC1, TC3: backward-compat: old atenea-arsenal marker blocks are also removed.
# Compatible with bash 3.2+.
set -euo pipefail

cd "$(dirname "$0")/.."
ARSENAL_ROOT="$(pwd)"
UNINSTALL_SH="$ARSENAL_ROOT/rtk/uninstall-rtk.sh"
...
test_tc1_marker_block_removed_preserves_context() {
  ...
  printf '%s\n' \
    '# pre-existing' \
    '# >>> atenea-arsenal rtk path >>>' \
    'case ":$PATH:" in *":$HOME/.arsenal/bin:"*) ;; *) export PATH="$HOME/.arsenal/bin:$PATH" ;; esac' \
    '# <<< atenea-arsenal rtk path <<<' \
    '# post-existing' \
    > "$tmpdir/.zshrc"

  # Need a valid state with installed_by_us=true so uninstall proceeds to marker removal
  # Use ~/.forge/bin/rtk (new path) — the old atenea-arsenal marker in .zshrc is the
  # backward-compat aspect being tested; only the shell-profile marker is the old format.
  mkdir -p "$tmpdir/.forge/bin"
  printf '#!/bin/sh\necho fake-rtk\n' > "$tmpdir/.forge/bin/rtk"
  chmod +x "$tmpdir/.forge/bin/rtk"
  ...
  # Backward-compat assertion: the old atenea-arsenal marker is still recognized and removed
  if grep -qF "# >>> atenea-arsenal rtk path >>>" "$tmpdir/.zshrc" 2>/dev/null; then
    fail "TC1: old atenea-arsenal marker should be absent after uninstall (backward-compat)"
  else
    pass "TC1: old atenea-arsenal marker is absent after uninstall (backward-compat)"
  fi
...
test_tc3_uninstall_idempotent() {
  ...
  # Backward-compat: old atenea-arsenal marker should be removed by first uninstall
  if grep -qF "# >>> atenea-arsenal rtk path >>>" "$tmpdir/.zshrc" 2>/dev/null; then
    fail "TC3: old atenea-arsenal marker should be absent after first uninstall (backward-compat)"
  else
    pass "TC3: old atenea-arsenal marker absent after first uninstall (backward-compat)"
  fi
