...
# _make_controlled_home <ARSENAL_ROOT_DIR>
# Creates a temp dir for HOME, creates <ARSENAL_ROOT_DIR>/rtk/VERSION containing
# 0.42.4, and returns the temp HOME path via stdout.
_make_controlled_home() {
  local arsenal_root_dir="$1"
  local tmphome
  tmphome="$(_make_tmpdir)"
  mkdir -p "$arsenal_root_dir/rtk"
  printf '0.42.4' > "$arsenal_root_dir/rtk/VERSION"
  echo "$tmphome"
}
...
_make_nocurl_bin_dir() {
  local bin_dir="$1"
  # Tools needed by arsenal_rtk_adjust_via_tarball (not curl, not uname, not shasum — caller adds those)
...
test_tarball_unsupported_platform() {
  ...
  local arsenal_root_fake="$tmpdir/arsenal_root"
  local tmphome
  tmphome="$(_make_controlled_home "$arsenal_root_fake")"
...
export ARSENAL_ROOT="$arsenal_root_fake"
export HOME="$tmphome"
...
# ---------------------------------------------------------------------------
# Test 6 — PATH warning when ~/.arsenal/bin not in PATH
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Test A1 — _arsenal_rtk_inject_path_snippet: existing profiles get marker block appended
# ---------------------------------------------------------------------------
