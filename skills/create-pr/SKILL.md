...
2. Refresh `[Unreleased]` and create the release commit deterministically:
   ...
   - Delegate to `applier` the execution of `$HOME/.claude/tools/release/commit-release.sh <BUMP> <NEXT>` (pass the `BUMP` and `NEXT` values parsed in step 1). The script stages the correct file set and creates the commit `chore(release): bump version to v<NEXT>`:
...
4. Delegate to `applier` the execution of `$HOME/.claude/tools/release/create-pr.sh` instead of improvising with raw `gh pr create` or REST API calls.
