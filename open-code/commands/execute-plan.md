...
- `FINDINGS_PHASE: impl=N, design=M[, coverage=K]`:
  ...
  - **After the batch of fixes is applied**, check `review_rounds`:
    - If `review_rounds < 1`: via `bash`, `set_frontmatter "<plan-path>" "review_rounds" "1"`, then fire EXACTLY ONE re-review. This re-review reads only the incremental diff `last_review_sha..HEAD` (not the full checkpoint range) and uses the SAME dispatch mechanism as above but with `model: anthropic/claude-sonnet` (or the repo's configured Sonnet-equivalent model) instead of `openai/gpt-5.6-sol` — the diff is small and bounded, so a lighter model suffices.
