...
forge_warn() {
  ...
  eval "_ARSENAL_WARN_MSG_${_ARSENAL_WARN_COUNT}=\$msg"
  eval "_ARSENAL_WARN_HINT_${_ARSENAL_WARN_COUNT}=\$hint"
  _ARSENAL_WARN_COUNT=$(( _ARSENAL_WARN_COUNT + 1 ))
}
...
forge_err() {
  ...
  [ -n "$hint" ] && echo "[forge]   → $hint" >&2
  eval "_ARSENAL_ERR_MSG_${_ARSENAL_ERR_COUNT}=\$msg"
  eval "_ARSENAL_ERR_HINT_${_ARSENAL_ERR_COUNT}=\$hint"
  _ARSENAL_ERR_COUNT=$(( _ARSENAL_ERR_COUNT + 1 ))
}
...
_forge_summarize_rtk() {
  ...
  local _has_rtk_hook
  _has_rtk_hook="$(jq -r '[.targets_manifest[]?.components[]? | select(. == "rtk-hook")] | length' "$ARSENAL_STATE_FILE" 2>/dev/null || echo "0")"
  local _rtk_tracked
  _rtk_tracked="$(jq -r '.rtk.tracked // false' "$ARSENAL_STATE_FILE" 2>/dev/null || echo "false")"
  ...
  local pinned=""
  if [ -f "$ARSENAL_ROOT/rtk/VERSION" ]; then
    pinned="$(cat "$ARSENAL_ROOT/rtk/VERSION" 2>/dev/null || true)"
  fi
  if [ -z "$pinned" ] && [ -f "${ARSENAL_STATE_FILE:-}" ]; then
    pinned=$(jq -r '.rtk.pinned_version // empty' "$ARSENAL_STATE_FILE" 2>/dev/null || true)
  fi
  ...
  case "$detected" in
    ...
    shadowed:*)
      ...
      echo "WARN|RTK ${_shadow_ver} instalado vía Homebrew sombrea al de arsenal (${_arsenal_ver})|Para solucionar:"
...
forge_print_summary() {
  ...
  total_warn=$(( _ARSENAL_WARN_COUNT + has_rtk_warn ))
  total_err=$_ARSENAL_ERR_COUNT
  ...
  if [ "$total_err" -gt 0 ]; then
    ...
    i=0
    while [ "$i" -lt "$_ARSENAL_ERR_COUNT" ]; do
      eval "msg=\${_ARSENAL_ERR_MSG_${i}}"
      eval "hint=\${_ARSENAL_ERR_HINT_${i}}"
      printf '  • %s\n' "$msg"
      [ -n "$hint" ] && printf '    → %s\n' "$hint"
      i=$(( i + 1 ))
    done
  ...
  if [ "$total_warn" -gt 0 ]; then
    ...
    i=0
    while [ "$i" -lt "$_ARSENAL_WARN_COUNT" ]; do
      eval "msg=\${_ARSENAL_WARN_MSG_${i}}"
      eval "hint=\${_ARSENAL_WARN_HINT_${i}}"
      printf '  • %s\n' "$msg"
      [ -n "$hint" ] && printf '    → %s\n' "$hint"
      i=$(( i + 1 ))
    done
...
source "$ARSENAL_ROOT/lib/symlink.sh"
# shellcheck source=/dev/null
source "$ARSENAL_ROOT/lib/json-merge.sh"
# shellcheck source=/dev/null
source "$ARSENAL_ROOT/lib/catalog.sh"
_rtk_lib="$ARSENAL_ROOT/lib/rtk.sh"
...
Subcommands:
  ...
  rtk uninstall Remove RTK if installed by arsenal
...
# ---------------------------------------------------------------------------
# Resolve targets
# Returns space-separated list of absolute paths in ARSENAL_TARGETS (global)
# Sets ARSENAL_TARGET_NAMES (parallel: "claude")
# ---------------------------------------------------------------------------
# We use two indexed arrays: _target_paths and _target_names
# Caller reads ARSENAL_TARGETS_COUNT, ARSENAL_TARGET_PATH_N, ARSENAL_TARGET_NAME_N
...
# Accepts an optional $1 path to override $ARSENAL_STATE_FILE (for testing).
...
_forge_state_migrate() {
  local _state_file="${1:-$ARSENAL_STATE_FILE}"
  ...
  if [ "$schema" -lt 2 ] 2>/dev/null; then
    ...
    while [ "$t" -lt "$targets_count" ]; do
      ...
      local pre_arsenal_path="${tgt_dir}/settings.json.pre-forge"
      local manifest_entry
      if [ -f "$pre_arsenal_path" ]; then
        manifest_entry="$(jq -n \
          --arg name "$tgt_name" \
          --arg dir "$tgt_dir" \
          --argjson symlinks "$rel_symlinks_json" \
          --arg settings_backup "$pre_arsenal_path" \
...
cmd_status() {
  ...
  if [ ! -f "$ARSENAL_STATE_FILE" ]; then
    echo "[forge] no instalado (state file no encontrado: $ARSENAL_STATE_FILE)"
    return 0
  fi
  ...
  if ! jq empty "$ARSENAL_STATE_FILE" 2>/dev/null; then
    echo "[forge] ERROR: state file inválido (JSON corrupto): $ARSENAL_STATE_FILE" >&2
    return 1
  fi

  local version installed_at
  version="$(jq -r '.version // "?"' "$ARSENAL_STATE_FILE")"
  installed_at="$(jq -r '.installed_at // "?"' "$ARSENAL_STATE_FILE")"
  local updated_at
  updated_at="$(jq -r '.updated_at // ""' "$ARSENAL_STATE_FILE")"
  ...
  # Targets
  local targets_str
  targets_str="$(jq -r '.targets // [] | join(", ")' "$ARSENAL_STATE_FILE")"
  ...
  manifest_count="$(jq -r '.targets_manifest // [] | length' "$ARSENAL_STATE_FILE")"
  ...
  _rtk_tracked_global="$(jq -r '.rtk.tracked // false' "$ARSENAL_STATE_FILE" 2>/dev/null || echo "false")"
  ...
  while [ "$t" -lt "$manifest_count" ]; do
    local tgt_name tgt_dir
    tgt_name="$(jq -r --argjson i "$t" '.targets_manifest[$i].name // "?"' "$ARSENAL_STATE_FILE")"
    tgt_dir="$(jq -r --argjson i "$t" '.targets_manifest[$i].dir // ""' "$ARSENAL_STATE_FILE")"

    echo "  Target: $tgt_name ($tgt_dir)"

    # Determine components to report. Fall back to full list if field is absent/empty.
    local _comp_count
    _comp_count="$(jq -r --argjson i "$t" '.targets_manifest[$i].components // [] | length' "$ARSENAL_STATE_FILE")"
    ...
    if [ "$_comp_count" -eq 0 ]; then
      ...
      while IFS= read -r _sc_comp; do
        eval "_STATUS_COMP_${t}_${_STATUS_COMP_COUNT}=\$_sc_comp"
        _STATUS_COMP_COUNT=$((_STATUS_COMP_COUNT + 1))
      done < <(jq -r --argjson i "$t" '.targets_manifest[$i].components[]' "$ARSENAL_STATE_FILE")
    fi
    ...
    while [ "$_ci" -lt "$_STATUS_COMP_COUNT" ]; do
      ...
      while IFS="	" read -r src_rel dest_rel; do
        ...
        if [ ! -e "$dest_abs" ] && [ ! -L "$dest_abs" ]; then
          ...
          if [ "${lnk_target#"$ARSENAL_ROOT"}" != "$lnk_target" ]; then
    ...
    local pre_arsenal="${target_settings}.pre-forge"
    ...
    if [ ! -f "$target_settings" ]; then
      ...
      [ ! -f "$pre_arsenal" ] && pre_status="ausente (no restaurable)"
      ...
      while [ "$_ci" -lt "$_STATUS_COMP_COUNT" ]; do
        ...
        local _manifest_path="$ARSENAL_ROOT/shared/components/${_scomp}.json"
    ...
    while [ "$_ci" -lt "$_STATUS_COMP_COUNT" ]; do
      ...
      local _mpath="$ARSENAL_ROOT/shared/components/${_mcomp}.json"
      if [ -f "$_mpath" ]; then
        local _ref
    ...
    if [ "$_print_rtk_block" -eq 1 ]; then
      ...
      if [ -f "$ARSENAL_ROOT/rtk/VERSION" ]; then
        rtk_pinned="$(cat "$ARSENAL_ROOT/rtk/VERSION" 2>/dev/null || true)"
      fi
      [ -z "${rtk_pinned:-}" ] && rtk_pinned="$(jq -r '.rtk.pinned_version // "?"' "$ARSENAL_STATE_FILE")"
      ...
      if command -v forge_rtk_detect >/dev/null 2>&1; then
        ...
        case "$_det" in
          ...
          installed:*)
            ...
            ;;
          shadowed:*)
            local _st_shadow_winner _st_shadow_ver _st_arsenal_ver
            _st_shadow_winner="$(printf '%s\n' "$_det" | cut -d: -f2)"
            _st_shadow_ver="$(printf '%s\n' "$_det" | cut -d: -f3)"
            _st_arsenal_ver="$(printf '%s\n' "$_det" | cut -d: -f4)"
            rtk_detected="sombra — ${_st_shadow_ver} en ${_st_shadow_winner} sombrea al de arsenal (${_st_arsenal_ver})"
            ;;
          *)         rtk_detected="$_det" ;;
        esac
      elif command -v rtk >/dev/null 2>&1; then
      ...
      case "$rtk_detected" in
        sombra\ —*)
          echo "    WARN: RTK instalado vía Homebrew sombrea al de arsenal."
...
cmd_update() {
  ...
  if git -C "$ARSENAL_ROOT" rev-parse --git-dir >/dev/null 2>&1; then
    local has_remote=0
    git -C "$ARSENAL_ROOT" remote 2>/dev/null | grep -q . && has_remote=1
    if [ "$has_remote" -eq 1 ]; then
      echo "[forge] git pull --ff-only ..."
      git -C "$ARSENAL_ROOT" pull --ff-only || echo "[forge] WARNING: git pull falló (modo local-only)"
    else
      echo "[forge] INFO: sin remote configurado, omitiendo git pull (modo local-only)"
    fi
  else
    echo "[forge] INFO: $ARSENAL_ROOT no es un repo git, omitiendo pull"
  fi
  ...
  if [ ! -f "$ARSENAL_STATE_FILE" ]; then
    ...
    upd_manifest_count="$(jq -r '.targets_manifest // [] | length' "$ARSENAL_STATE_FILE")"
    ...
    if [ "$upd_manifest_count" -gt 0 ]; then
      ...
      while [ "$upd_t" -lt "$upd_manifest_count" ]; do
        ...
        upd_tgt_name="$(jq -r --argjson i "$upd_t" '.targets_manifest[$i].name' "$ARSENAL_STATE_FILE")"
        upd_tgt_dir="$(jq -r --argjson i "$upd_t" '.targets_manifest[$i].dir' "$ARSENAL_STATE_FILE")"
        ...
        _upd_components_count="$(jq -r --argjson i "$upd_t" \
          '.targets_manifest[$i].components // [] | length' "$ARSENAL_STATE_FILE")"
        ...
        if [ "$_upd_components_count" -eq 0 ]; then
          ...
          while IFS= read -r _uc_comp; do
            eval "_UPDCOMP_${upd_t}_${_uc_count}=\$_uc_comp"
            _uc_count=$((_uc_count + 1))
          done < <(jq -r --argjson i "$upd_t" '.targets_manifest[$i].components[]' "$ARSENAL_STATE_FILE")
        fi
        ...
        while [ "$_ui" -lt "$_uc_count" ]; do
          ...
          _upd_claude_md_ref="$(jq -r '.claude_md_ref // empty' \
            "$ARSENAL_ROOT/shared/components/${_ucomp}.json" 2>/dev/null || true)"
  ...
  if [ -f "$ARSENAL_STATE_FILE" ] && jq empty "$ARSENAL_STATE_FILE" 2>/dev/null; then
    local sc_manifest_count sc_t
    sc_manifest_count="$(jq -r '.targets_manifest // [] | length' "$ARSENAL_STATE_FILE")"
    sc_t=0
    while [ "$sc_t" -lt "$sc_manifest_count" ]; do
      local sc_name sc_dir
      sc_name="$(jq -r --argjson i "$sc_t" '.targets_manifest[$i].name' "$ARSENAL_STATE_FILE")"
      sc_dir="$(jq -r --argjson i "$sc_t" '.targets_manifest[$i].dir' "$ARSENAL_STATE_FILE")"
  ...
  if [ -f "$ARSENAL_STATE_FILE" ] && jq empty "$ARSENAL_STATE_FILE" 2>/dev/null; then
    ...
    jq --arg upd "$upd_at" '. + {updated_at: $upd}' "$ARSENAL_STATE_FILE" > "$upd_tmp"
    # shellcheck disable=SC2015  # intentional: A && B || C used for atomic write-or-cleanup
    jq empty "$upd_tmp" && mv "$upd_tmp" "$ARSENAL_STATE_FILE" || rm -f "$upd_tmp"
...
# If CLAUDE.md is itself an arsenal-owned symlink the symlink is removed instead
...
_forge_strip_claude_md_ref() {
  ...
  _scr_ref="$(jq -r '.claude_md_ref // empty' \
    "$ARSENAL_ROOT/shared/components/${_scr_component}.json" 2>/dev/null || true)"
  [ -n "$_scr_ref" ] || return 0
...
cmd_uninstall() {
  ...
  if [ -n "$component_name" ]; then
    ...
    _cu_in_state="$(jq -r --arg comp "$component_name" \
      ...
      "$ARSENAL_STATE_FILE" 2>/dev/null || echo "0")"
    if [ "$_cu_in_state" = "0" ] || [ -z "$_cu_in_state" ]; then
      echo "[forge] ERROR: el componente '$component_name' no está instalado en ningún target" >&2
      echo "[forge]   → componentes instalados: $(jq -r '[.targets_manifest[]?.components[]?] | unique | .[]' "$ARSENAL_STATE_FILE" 2>/dev/null | tr '\n' ',' | sed 's/,$//')" >&2
      return 1
    fi
    ...
    _su_manifest_count="$(jq -r '.targets_manifest // [] | length' "$ARSENAL_STATE_FILE")"
    ...
    while [ "$_su_snap_i" -lt "$_su_manifest_count" ]; do
      eval "_su_snap_name_${_su_snap_i}=\"$(jq -r --argjson i "$_su_snap_i" '.targets_manifest[$i].name' "$ARSENAL_STATE_FILE")\""
      eval "_su_snap_dir_${_su_snap_i}=\"$(jq -r --argjson i "$_su_snap_i" '.targets_manifest[$i].dir' "$ARSENAL_STATE_FILE")\""
    ...
    while [ "$_su_t" -lt "$_su_manifest_count" ]; do
      ...
      _su_trf_count="$(jq -r '.target_root_files | length' \
        "$ARSENAL_ROOT/shared/components/${component_name}.json" 2>/dev/null || echo "0")"

      local _su_trf_i=0
      while [ "$_su_trf_i" -lt "$_su_trf_count" ]; do
        local _su_trf_dest
        _su_trf_dest="$(jq -r ".target_root_files[$_su_trf_i].dest" \
          "$ARSENAL_ROOT/shared/components/${component_name}.json" 2>/dev/null)"
      ...
      _su_remaining_components="$(jq -c --arg tgt "$_su_tgt_name" --arg comp "$component_name" \
        ...
        "$ARSENAL_STATE_FILE" 2>/dev/null || echo "[]")"
      ...
      if [ "${_su_remaining_count:-0}" = "0" ]; then
        ...
        jq --arg tgt "$_su_tgt_name" \
          ...
          "$ARSENAL_STATE_FILE" > "$_su_tmp_state"
        if jq empty "$_su_tmp_state" 2>/dev/null; then
          mv "$_su_tmp_state" "$ARSENAL_STATE_FILE"
        ...
        _su_remaining_targets="$(jq -r '.targets_manifest // [] | length' "$ARSENAL_STATE_FILE" 2>/dev/null || echo "0")"
        if [ "${_su_remaining_targets:-0}" = "0" ]; then
          rm -f "$ARSENAL_STATE_FILE"
        ...
        _su_comp_dests="$(jq -c '[.symlinks[].dest, .target_root_files[].dest]' \
          "$ARSENAL_ROOT/shared/components/${component_name}.json" 2>/dev/null || echo "[]")"
  ...
  local _rs_t_count
  _rs_t_count="$(jq -r '.targets_manifest // [] | length' "$ARSENAL_STATE_FILE")"
  local _rs_t=0
  while [ "$_rs_t" -lt "$_rs_t_count" ]; do
    local _rs_tgt_dir
    _rs_tgt_dir="$(jq -r --argjson i "$_rs_t" '.targets_manifest[$i].dir // empty' "$ARSENAL_STATE_FILE")"
    local _rs_dest
    ...
    done < <(jq -r --argjson i "$_rs_t" '.targets_manifest[$i].symlinks // [] | .[]' "$ARSENAL_STATE_FILE")
    _rs_t=$((_rs_t + 1))
  done
  ...
  local _fu_t_count
  _fu_t_count="$(jq -r '.targets_manifest // [] | length' "$ARSENAL_STATE_FILE" 2>/dev/null || echo "0")"
  local _fu_t=0
  while [ "$_fu_t" -lt "$_fu_t_count" ]; do
    local _fu_tgt_dir
    _fu_tgt_dir="$(jq -r --argjson i "$_fu_t" '.targets_manifest[$i].dir' "$ARSENAL_STATE_FILE")"
    if [ -n "$_fu_tgt_dir" ] && [ "$_fu_tgt_dir" != "null" ]; then
      local _fu_comp
      while IFS= read -r _fu_comp; do
        [ -z "$_fu_comp" ] && continue
        _forge_strip_claude_md_ref "$_fu_comp" "$_fu_tgt_dir"
      done < <(jq -r --argjson i "$_fu_t" '.targets_manifest[$i].components // [] | .[]' "$ARSENAL_STATE_FILE")
  ...
  # The binary lives in arsenal's private dir (~/.forge); a full uninstall
  ...
  settings_targets="$(jq -r '.settings.settings_json_backup // {} | keys[]' "$ARSENAL_STATE_FILE" 2>/dev/null || true)"
  ...
  for tgt_name in $settings_targets; do
    ...
    if [ -f "$pre_arsenal" ]; then
      ...
      local overlay_json
      overlay_json="$(jq -c --arg n "$tgt_name" '.settings.overlay_backup[$n] // null' "$ARSENAL_STATE_FILE" 2>/dev/null || echo "null")"
      ...
      if [ "$overlay_json" != "null" ] && [ -n "$overlay_json" ]; then
        ...
        if [ -f "$sfile" ] && jq empty "$sfile" 2>/dev/null; then
          ...
          managed_paths_json="$(jq -c '.settings.managed_paths // []' "$ARSENAL_STATE_FILE")"
          local ov_tmp
          ov_tmp="$(mktemp)"
  ...
  while [ "$t" -lt "$manifest_count" ]; do
    ...
    while [ "$_ci" -lt "$_rc_count" ]; do
      ...
      _claude_md_ref="$(jq -r '.claude_md_ref // empty' \
        "$ARSENAL_ROOT/shared/components/${_comp}.json" 2>/dev/null || true)"
...
cmd_doctor() {
  ...
  # Falls back to full component list for pre-v3 state entries lacking components field.
  if [ ! -f "$ARSENAL_STATE_FILE" ]; then
    _doc_warn "state file no encontrado ($ARSENAL_STATE_FILE) — no instalado"
  elif ! jq empty "$ARSENAL_STATE_FILE" 2>/dev/null; then
    _doc_fail "state file corrupto: $ARSENAL_STATE_FILE"
  else
    _forge_state_migrate
    _doc_pass "state file válido"

    local _doc_manifest_count
    _doc_manifest_count="$(jq -r '.targets_manifest // [] | length' "$ARSENAL_STATE_FILE")"

    local _doc_rtk_hook_present=0
    local _doc_rtk_tracked
    _doc_rtk_tracked="$(jq -r '.rtk.tracked // false' "$ARSENAL_STATE_FILE" 2>/dev/null || echo "false")"
    local _dt=0
    while [ "$_dt" -lt "$_doc_manifest_count" ]; do
      local _dtgt_name _dtgt_dir
      _dtgt_name="$(jq -r --argjson i "$_dt" '.targets_manifest[$i].name // "?"' "$ARSENAL_STATE_FILE")"
      _dtgt_dir="$(jq -r --argjson i "$_dt" '.targets_manifest[$i].dir // ""' "$ARSENAL_STATE_FILE")"
      ...
      _doc_comp_count="$(jq -r --argjson i "$_dt" '.targets_manifest[$i].components // [] | length' "$ARSENAL_STATE_FILE")"
      ...
      if [ "$_doc_comp_count" -eq 0 ]; then
        ...
        while IFS= read -r _dsc_comp; do
          eval "_DOC_COMP_${_dt}_${_DOC_COMP_COUNT}=\$_dsc_comp"
          _DOC_COMP_COUNT=$((_DOC_COMP_COUNT + 1))
        done < <(jq -r --argjson i "$_dt" '.targets_manifest[$i].components[]' "$ARSENAL_STATE_FILE")
      fi
      ...
      while [ "$_doc_ci" -lt "$_DOC_COMP_COUNT" ]; do
        ...
        while IFS="	" read -r _dsrc_rel _ddest_rel; do
          ...
          if [ ! -e "$_ddest_abs" ] && [ ! -L "$_ddest_abs" ]; then
            ...
            if [ "${_dlnk_target#"$ARSENAL_ROOT"}" != "$_dlnk_target" ]; then
      ...
      local _dpre_arsenal="${_dsfile}.pre-forge"
      ...
      if [ ! -f "$_dsfile" ]; then
        ...
        while [ "$_doc_ci" -lt "$_DOC_COMP_COUNT" ]; do
          ...
          local _dmanifest_path="$ARSENAL_ROOT/shared/components/${_dscomp}.json"
      ...
      while [ "$_doc_ci" -lt "$_DOC_COMP_COUNT" ]; do
        ...
        local _drmanifest="$ARSENAL_ROOT/shared/components/${_drefcomp}.json"
...
# to ARSENAL_ROOT and all *.forge-bak-* files from agents/, commands/ and
...
_forge_clean_target() {
  ...
  if [ "$#" -eq 0 ]; then
    ...
    for scan_dir in "$tgt_dir/agents" "$tgt_dir/commands" "$tgt_dir/skills" "$tgt_dir"; do
      [ -d "$scan_dir" ] || continue
      # Remove symlinks pointing to ARSENAL_ROOT
      while IFS= read -r -d '' f; do
        local lnk_target
        lnk_target="$(readlink "$f")"
        case "$lnk_target" in
          "$ARSENAL_ROOT"/*) ;;  # owned by arsenal — proceed to remove
...
cmd_install() {
  ...
  while [ "$_cf_i" -lt "$_selected_count" ]; do
    ...
    if [ -f "$ARSENAL_STATE_FILE" ] && jq empty "$ARSENAL_STATE_FILE" 2>/dev/null; then
      ...
      done < <(jq -r '[.targets_manifest[]?.components[]?] | unique | .[]' "$ARSENAL_STATE_FILE" 2>/dev/null)
    fi
  ...
  while [ "$i" -lt "$_forge_targets_count" ]; do
    ...
    # Only removes entries that are arsenal-managed symlinks (pointing into
    # ARSENAL_ROOT); silently skips absent or non-arsenal entries. Idempotent.
    # ---------------------------------------------------------------------------
    ...
    while [ "$_comp_i" -lt "$_selected_count" ]; do
      ...
      local _claude_md_ref
      _claude_md_ref="$(jq -r '.claude_md_ref // empty' "$ARSENAL_ROOT/shared/components/${_comp_name}.json" 2>/dev/null || true)"
      if [ -n "$_claude_md_ref" ]; then
  ...
  while [ "$m" -lt "$_forge_targets_count" ]; do
    ...
    local pre_arsenal_path="${tgt}/settings.json.pre-forge"
    local backup_path=""
    [ -f "$pre_arsenal_path" ] && backup_path="$pre_arsenal_path"
    ...
    if [ -f "$ARSENAL_STATE_FILE" ]; then
      existing_overlay="$(jq -c --arg n "$tgt_name" '.settings.overlay_backup[$n] // null' "$ARSENAL_STATE_FILE" 2>/dev/null || echo "null")"
    fi
    ...
    local pre_arsenal_check="${tgt}/settings.json.pre-forge"
    if [ -f "$pre_arsenal_check" ]; then
      settings_backup_val="$pre_arsenal_check"
    else
      settings_backup_val=""
    fi
  ...
  if [ -f "$ARSENAL_STATE_FILE" ]; then
    local existing_at
    existing_at="$(jq -r '.installed_at // empty' "$ARSENAL_STATE_FILE" 2>/dev/null || true)"
  ...
  pinned_version="$(cat "$ARSENAL_ROOT/rtk/VERSION" 2>/dev/null || echo "0.42.4")"
  ...
  mv "$tmp_state" "$ARSENAL_STATE_FILE"
  echo "[forge] state guardado: $ARSENAL_STATE_FILE"
        ...
        install)
          ...
          if [ "${_RTK_INSTALL_FAILED:-}" != "1" ] && [ "${_RTK_VERSION_MISMATCH:-}" != "1" ]; then
            if [ -f "$ARSENAL_STATE_FILE" ] && jq empty "$ARSENAL_STATE_FILE" 2>/dev/null; then
              ...
              jq '.rtk.tracked = true' "$ARSENAL_STATE_FILE" > "$_rtk_track_tmp"
              # shellcheck disable=SC2015  # intentional A && B || C for atomic write-or-cleanup
              jq empty "$_rtk_track_tmp" && mv "$_rtk_track_tmp" "$ARSENAL_STATE_FILE" || rm -f "$_rtk_track_tmp"
              echo "[rtk] tracking activado (rtk.tracked=true en state)"
            else
              echo "[rtk] tracking no activado: no hay instalación de arsenal registrada — ejecuta 'bash install.sh --only=core,statusline' y después repite 'bash install.sh rtk install'"
            fi
          else
          ...
          ;;
        uninstall)
          local rtk_uninstall="$ARSENAL_ROOT/rtk/uninstall-rtk.sh"
          ...
          if [ -f "$ARSENAL_STATE_FILE" ] && jq empty "$ARSENAL_STATE_FILE" 2>/dev/null; then
            ...
            jq 'if .rtk then .rtk = (.rtk | del(.tracked)) else . end' "$ARSENAL_STATE_FILE" > "$_rtk_untrack_tmp"
            # shellcheck disable=SC2015  # intentional A && B || C for atomic write-or-cleanup
            jq empty "$_rtk_untrack_tmp" && mv "$_rtk_untrack_tmp" "$ARSENAL_STATE_FILE" || rm -f "$_rtk_untrack_tmp"
