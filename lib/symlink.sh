...
forge_unlink() {
  ...
  if [ -L "$dest" ]; then
    ...
    local pre_arsenal="${dest}.pre-forge"
    if [ -e "$pre_arsenal" ]; then
      mv "$pre_arsenal" "$dest" || { echo "[symlink] ERROR: cannot restore $pre_arsenal" >&2; forge_err "symlink: cannot restore $pre_arsenal"; return 1; }
