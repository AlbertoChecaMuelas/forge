...
if [ -n "${_FORGE_RTK_LOADED:-}" ]; then
  return 0
fi
_FORGE_RTK_LOADED=1

# Fallback no-op if forge_warn is not loaded yet
...
forge_rtk_strip_path_snippet() {
  ...
  for _profile in "$HOME/.zshrc" "$HOME/.bashrc" "$HOME/.zprofile" "$HOME/.bash_profile"; do
    [ -f "$_profile" ] || continue
    grep -qF '# >>> forge rtk path >>>' "$_profile" || continue
    _profile_tmp="$(mktemp)"
    sed '/# >>> forge rtk path >>>/,/# <<< forge rtk path <<</d' \
...
forge_rtk_adjust_via_tarball() {
  ...
  local pinned
  pinned="$(cat "$FORGE_ROOT/rtk/VERSION" 2>/dev/null | tr -d '[:space:]')"
  if [ -z "$pinned" ]; then
    forge_warn "[rtk] No se pudo leer la versión pinned de rtk/VERSION"
  ...
  case "${_os}-${_arch}" in
    ...
    *)
      forge_warn "[rtk] Plataforma no soportada: ${_os}-${_arch}"
      _RTK_INSTALL_FAILED=1
      return 1
      ;;
  esac
  ...
  if ! command -v curl >/dev/null 2>&1; then
    forge_warn "[rtk] curl no encontrado; instálalo para continuar."
    _RTK_INSTALL_FAILED=1
    return 1
  fi
  ...
  if ! curl -fsSL --retry 3 "${_base_url}/${_asset}" -o "$_tmpdir/${_asset}"; then
    forge_warn "[rtk] Error al descargar ${_asset}"
    rm -rf "$_tmpdir"
    _RTK_INSTALL_FAILED=1
    return 1
  fi
  if ! curl -fsSL --retry 3 "${_base_url}/checksums.txt" -o "$_tmpdir/checksums.txt"; then
    forge_warn "[rtk] Error al descargar checksums.txt"
    rm -rf "$_tmpdir"
    _RTK_INSTALL_FAILED=1
    return 1
  fi
  ...
  if [ -z "$_expected_sha" ]; then
    forge_warn "[rtk] No se encontró el checksum para ${_asset} en checksums.txt"
    rm -rf "$_tmpdir"
    _RTK_INSTALL_FAILED=1
    return 1
  fi
  if command -v shasum >/dev/null 2>&1; then
    ...
    forge_warn "[rtk] No se encontró shasum ni sha256sum para verificar el checksum"
    rm -rf "$_tmpdir"
    _RTK_INSTALL_FAILED=1
    return 1
  fi
  if [ "$_actual_sha" != "$_expected_sha" ]; then
    forge_warn "[rtk] Checksum SHA256 no coincide para ${_asset} (esperado: ${_expected_sha}, obtenido: ${_actual_sha})"
    rm -rf "$_tmpdir"
    _RTK_INSTALL_FAILED=1
    return 1
  fi
  ...
  if ! tar -xzf "$_tmpdir/${_asset}" -C "$_tmpdir"; then
    forge_warn "[rtk] Error al extraer el tarball ${_asset}"
    rm -rf "$_tmpdir"
    _RTK_INSTALL_FAILED=1
    return 1
  fi
  ...
  if [ -z "$_binary" ]; then
    forge_warn "[rtk] No se encontró el binario rtk en el tarball"
    rm -rf "$_tmpdir"
    _RTK_INSTALL_FAILED=1
    return 1
  fi
  ...
  case ":${PATH}:" in
    *":$HOME/.forge/bin:"*) ;;
    *)
      forge_warn "[rtk] $HOME/.forge/bin no está en PATH → Añade $HOME/.forge/bin a tu PATH en ~/.zshrc o ~/.bashrc"
      ;;
  esac
  ...
  if [ "$_installed_ver" != "$pinned" ]; then
    forge_warn "[rtk] El binario instalado reporta versión ${_installed_ver}, esperada ${pinned}"
    rm -rf "$_tmpdir"
    _RTK_INSTALL_FAILED=1
    return 1
  fi
...
# Shared helper: reads FORGE_RTK_ADJUST env var (yes|no), falls back to tty prompt.
# Backward-compat: FORGE_RTK_DOWNGRADE is still accepted as a deprecated alias.
# If both are set, FORGE_RTK_ADJUST takes precedence. Used for both
...
_forge_rtk_prompt_adjust() {
  ...
  # Resolve env var: FORGE_RTK_ADJUST is primary; FORGE_RTK_DOWNGRADE is
  # a deprecated alias kept for backward compatibility. Primary wins if both set.
  local env_choice="${FORGE_RTK_ADJUST:-${FORGE_RTK_DOWNGRADE:-}}"
  local env_var_name="FORGE_RTK_ADJUST"
  if [ -z "${FORGE_RTK_ADJUST:-}" ] && [ -n "${FORGE_RTK_DOWNGRADE:-}" ]; then
    env_var_name="FORGE_RTK_DOWNGRADE"
    echo "[rtk] WARN: FORGE_RTK_DOWNGRADE está deprecado; usa FORGE_RTK_ADJUST" >&2
  fi
  case "$env_choice" in
    yes)
...
forge_rtk_decide() {
  ...
  if [ "$detected" = "collision" ]; then
    ...
    _RTK_INSTALL_FAILED=1
    forge_warn "RTK: colisión con otro binario 'rtk' en PATH (probable Rust Type Kit)" "Desinstala el otro binario o ajusta PATH; luego: bash install.sh rtk install"
    return 0
  fi
  ...
  case "$detected" in
    shadowed:*)
      ...
      _forge_ver="$(printf '%s\n' "$detected" | cut -d: -f4)"
      forge_warn "RTK ${_shadow_ver} instalado vía Homebrew sombrea al de forge (${_forge_ver})." \
        "Para solucionar:
  ...
  if [ "$cmp" = "lt" ]; then
    ...
    echo "[rtk]   bash install.sh rtk install" >&2
    forge_warn "RTK $detected detectado (upgrade automático deshabilitado)" "bash install.sh rtk install"
  ...
  if [ "$cmp" = "gt" ]; then
    ...
    forge_warn "RTK $detected > pin $pinned (versión no certificada)" "bash install.sh rtk install"
