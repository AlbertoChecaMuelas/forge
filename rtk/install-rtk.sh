...
_write_rtk_state() {
  ...
  if [ -f "$FORGE_STATE_FILE" ]; then
    ...
    local tmp
    tmp="$(mktemp)"
    jq --argjson rtk "$rtk_section" '.rtk = $rtk' "$FORGE_STATE_FILE" > "$tmp"
    ...
    echo "[rtk] state actualizado: $FORGE_STATE_FILE"
  else
    echo "[rtk] state file no encontrado ($FORGE_STATE_FILE); state RTK no persistido"
  fi
}
