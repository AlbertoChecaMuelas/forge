...
orch_part=""
if [ -f "$HOME/.claude/.forge-orchestrator-active" ] && [ -n "$session_id" ] && [ "$session_id" != "null" ]; then
  _orch_sid=$(cat "$HOME/.claude/.forge-orchestrator-active" 2>/dev/null)
...
show_cost=0
[ -f "${TARGET_DIR}/.forge-show-cost" ] && show_cost=1
...
# Línea 3 (lifetime stats) solo si el fichero centinela .forge-show-cost está presente
